...
**核心规则：每拉 1 个新用户 = 你 +1 次免费深度解析（AI 解读额度）**

1. 推荐人在网站首页点「复制邀请链接」或进「推广活动」拿专属链接 `?ref=XXXX`
2. 好友点链接进入网站 → 首页出现「新人礼」横幅 → 点「立即领取」**创建账户完成注册**
3. 注册瞬间自动触发：**你和好友各 +1 次免费深度解析**（好友的立即到账，你的计入推广页余额，可随时兑换）
4. 一个设备只计一次、自荐无效、无需好友付费，多邀多得上不封顶
5. 推荐人在「推广活动」页可查看：**已带来 N 位好友 / 可用深度解析 N 次**；额度返利默认自动到账，无需任何设置

**补充：好友购买会员时的额外奖励**

- 好友通过你的链接下单并确认到账后，`orders` 表置 paid 会自动再生成一笔返利：
  - 你选 `解读额度`（默认）：自动 +1 次，直接计入推广页余额
  - 你选 `现金`：按 `settings.rebate_rate`（默认 30%）在 **Table Editor → rebates** 记一笔 pending，
    你人工转账后把 status 改成 `paid` 留底；不想返就改 `skipped`
- 推荐人没绑收款账号的，note 列会提示你去找 TA 确认

**运营提示**：拉新是纯免费奖励（AI 深度解读走用户自己的 API Key，不产生你的成本），
适合放手让用户去裂变；现金返利才涉及真金白银，默认已关闭（推荐人需主动切换到现金并填收款账号）。
...
| 查拉新数据 | Table Editor → signup_claims（每个注册的新用户一行，referrer 列是推荐码） |
